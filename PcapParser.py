    ...
    def split(self, split_size: int=None, target_csv_dir: str=None, pcap_path: str=None, base_filter: str='ip'):
        ...
        for packet in cap:
            ...
            if file_count == split_size:
                ...
                target_file = out_dir + '/' + filename + '_' + str(file_count // split_size) + '.csv'
                self.__parse(packet_list=packet_list, out_file=target_file)
                print(">> parse {} packet data into {}".format(split_size, target_file))
        ...
        if packet_list is not []:
            ...
            target_file = out_dir + '/' + filename + '_' + str(file_count // split_size) + '.csv'
            self.__parse(packet_list=packet_list, out_file=target_file)
            print(">> parse {} packet data into {}".format(len(packet_list), target_file))
            # self.__tocsv(attr_tuple=attr_tuple, out_file=target_file)

    def __parse(self, packet_list: list, out_file: str):
        if not packet_list:
            return
        for pkt in packet_list:
            ...
            self.__tocsv((ip_params, tcp_params, udp_params, tls_params), out_file=out_file)
        # return (ip_params, tcp_params, udp_params, tls_params)
        
    def __tocsv(self, attr_tuple: tuple, out_file: str):
        fmt = "{}," * 29
        with open(out_file, 'a') as csvf:
            csvf.write(
                fmt.format(
